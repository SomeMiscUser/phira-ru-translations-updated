label = 譜面庫
local = 本地
popular = 熱門
page = 第 { $current } / { $total } 頁
prev-page = 上一頁
next-page = 下一頁
not-opened = 敬請期待
failed-to-load-online = 載入線上譜面失敗
import = 匯入
offline-mode = 離線模式下無法載入線上譜面
must-login = 登入才可查看線上譜面
order-by = 排序方式：{ $order }
order-asc = 升序
order-desc = 降序
order-updated = 排序已更新
order-update-failed-sort = 這能在預設排序方式下更新排序
fav-synced = 我的最愛已同步
fav-sync-failed = 我的最愛同步失敗
multi-select-status = 已選中 { $count } 項
multi-export = 打包多個譜面
multi-create-fav = 加入到新資料夾
multi-manage-fav = 管理資料夾
multi-delete = 刪除
multi-delete-confirm = 確定要刪除這 { $count } 個譜面嗎？該操作無法撤銷。
multi-deleted = 已刪除
multi-export-title = 選擇匯出位置
multi-export-no-file = 請在匯出前下載所有選中的譜面。未下載：{ $charts }
multi-exporting = 匯出中 ({ $current } / { $total })
exported = 已匯出
multi-added-to-fav = 已加入到資料夾
multi-removed-from-fav = 已從資料夾移除
multi-select-all = 選取全部
multi-select-invert = 反向選取
fav-created = 資料夾建立成功
